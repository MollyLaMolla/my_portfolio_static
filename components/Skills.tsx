...
const Skills = () => {
    ...
    () =>
      ({
        ...
        transition:
          'transform 0.6s cubic-bezier(0.22, 1, 0.36, 1), box-shadow 0.6s cubic-bezier(0.22, 1, 0.36, 1), background-color 0.6s cubic-bezier(0.22, 1, 0.36, 1)',
        willChange: 'transform',
        boxShadow: '0 8px 30px rgba(139,92,246,0.25), 0 0 40px rgba(59,130,246,0.15)',
  ...
  const baseTransform = useMemo(
    () =>
      ({
        transform: 'perspective(900px) translateZ(0px) rotateX(0deg) rotateY(0deg)',
        transition:
          'transform 0.6s cubic-bezier(0.22, 1, 0.36, 1), box-shadow 0.6s cubic-bezier(0.22, 1, 0.36, 1), background-color 0.6s cubic-bezier(0.22, 1, 0.36, 1)',
        willChange: 'transform',
      }) as React.CSSProperties,
  ...
  return (
    <div className="py-24 relative flex justify-center items-center flex-col" id="tech-stack">
      ...
      >
        {skills.map((skill, index) => {
          ...
          if (!isTouch) {
            ...
            else if (isNeighbor(index, hoverIdx)) style = tiltFor(index, hoverIdx as number)
          }
          ...
          return (
            ...
            >
              ...
              {(() => {
                const isN = hoverIdx !== null && hoverIdx !== index && isNeighbor(index, hoverIdx)
                const A = indexToRC(index)
                const B = hoverIdx !== null ? indexToRC(hoverIdx) : A
                const dr = A.r - B.r
                const dc = A.c - B.c
                const shift = 6
                const tx = isN ? (dc === -1 ? +shift : dc === 1 ? -shift : 0) : 0
                const ty = isN ? (dr === -1 ? +shift : dr === 1 ? -shift : 0) : 0
